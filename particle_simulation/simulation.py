...
class Simulation(object):
    ...
    def __init__(self, *,
        ...
        self.init_c = init_c
        self._images = np.zeros(map_size.shape)

        self._coords = np.array(np.meshgrid(*[np.arange(self.x_size) for _ in range(self.dim)]))

        self.structure_factor = np.zeros_like(self._images)

    def save_h5(self, name: str,
                rdr: List[Tuple[float, float]] = None,
                save_images: bool = True,
                save_structure_factor: bool = False,
                mode: str = 'std') -> bool:
        try:
            with h5py.File(self.H5FilePath, 'a') as f:
                group = f.create_group(name)
                group.attrs.update(self.map_size.asdict())
                if save_images:
                    group.create_dataset('images', data=self._images)
                if save_structure_factor:
                    group.create_dataset('structure_factor',
                                         data=self.structure_factor)
                group.create_dataset('c', data=self.c)
                group.create_dataset('radius', data=self.radius)
                group.create_dataset('centers', data=self.centers)
                if rdr is not None:
                    for i, (r, dr) in enumerate(rdr):
                        corr_func = self.get_two_time_corr_function(r, dr,
                                                                    mode)
                        dset = group.create_dataset(f'xpcs_{i}', data=corr_func)
                        dset.attrs['r'] = r
                        dset.attrs['dr'] = dr
            return True
    ...
    def _get_mean_sf(self, q, dq):
        ...
        for t in range(self.t_size):
            kinetics[t] = self.structure_factor[t, mask].mean()
        return kinetics
    ...
    def run(self, *,
        ...
        for t in tqdm(np.arange(self.t_size)):
            ...
            self._images[t] = image
            self.structure_factor[t] = np.abs(fftshift(fftn(image)))
    ...
    def get_mask(self, q: float, dq: float):
        ...
        number_of_pixels = self.structure_factor[0, mask].size
    ...
    def get_q_array(self,
        ...
        q_array = self.structure_factor[:, mask].T
    ...
    def plot_xpcs(self, q, dq, mode: str = 'std',
                  show: bool = True):
        xpcs = self.get_two_time_corr_function(q, dq, mode)
        if xpcs is None:
            return
        plt.imshow(xpcs, cmap='jet', origin='lower', vmin=0, vmax=1)
